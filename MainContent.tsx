...
// Updated Trend Badge: 
// 1. Icon is RED
// 2. Border is RGB Animated (Changing colors)
// 3. No text, just icon inside the frame
  ...
  return (
    <div className="absolute top-2 left-2 z-50">
      <style>
        {`
          @keyframes neonRGB {
            0% { border-color: #ef4444; box-shadow: 0 0 10px #ef4444, inset 0 0 5px #ef4444; } /* Red */
            20% { border-color: #eab308; box-shadow: 0 0 10px #eab308, inset 0 0 5px #eab308; } /* Yellow */
            40% { border-color: #22d3ee; box-shadow: 0 0 10px #22d3ee, inset 0 0 5px #22d3ee; } /* Cyan */
            60% { border-color: #d946ef; box-shadow: 0 0 10px #d946ef, inset 0 0 5px #d946ef; } /* Fuchsia */
            80% { border-color: #10b981; box-shadow: 0 0 10px #10b981, inset 0 0 5px #10b981; } /* Emerald */
            100% { border-color: #ef4444; box-shadow: 0 0 10px #ef4444, inset 0 0 5px #ef4444; } /* Red */
          }
...
const VideoCardThumbnail: React.FC<{ 
  ...
  return (
    // Background remains black (neutral-950). Border color comes from neonStyle which varies per video.
    <div className={`w-full h-full relative bg-neutral-950 overflow-hidden group rounded-2xl border-2 transition-all duration-500 ${neonStyle} ${video.is_trending ? 'scale-[1.03] border-red-600 shadow-[0_0_20px_#dc2626]' : 'hover:scale-[1.01]'}`}>
      ...
      {/* Trend Badge - Kept in Grid View with Red Icon & RGB Border */}
      <NeonTrendBadge is_trending={video.is_trending} />

      <div className="absolute top-2 right-2 flex flex-col items-center gap-2 z-30">
        <button 
        ...
        {!isWatched && (
          <div className="px-2 py-0.5 bg-blue-600/10 rounded-md border border-blue-500/30 shadow-[0_0_15px_rgba(59,130,246,0.5)] backdrop-blur-sm animate-pulse">
            <span className="text-[8px] font-black text-blue-400 drop-shadow-[0_0_5px_rgba(59,130,246,0.8)]">جديد</span>
          </div>
        )}
      </div>
  ...
  const animate = useCallback(() => {
    ...
    if (container && !isDragging) {
      ...
      if (scrollWidth > 0) {
        ...
        if (internalSpeed > 0) { // Right to Left
...
const MainContent: React.FC<any> = ({ 
  ...
  const getRandomPosition = () => {
    const top = Math.floor(Math.random() * 60) + 15 + '%'; // 15% to 75% vertical
    const left = Math.floor(Math.random() * 40) + 5 + '%'; // 5% to 45% horizontal
    const animations = [
        'translate(100px, 0)', 
        'translate(-100px, 0)', 
        'translate(0, 100px)', 
  ...
  return (
    ...
    >
      <header className="flex items-center justify-between py-2 bg-black relative px-4 border-b border-white/5 shadow-lg h-14">
        <div className="flex items-center gap-2" onClick={onHardRefresh}>
          ...
          <h1 className={`text-sm font-black italic transition-colors duration-500 ${isActuallyRefreshing ? 'text-yellow-400' : 'text-red-600'}`}>الحديقة المرعبة</h1>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => setIsSearchOpen(true)} className="p-2 bg-white/5 border border-white/10 text-white/70 hover:text-white transition-all active:scale-90 rounded-xl shadow-md">
      ...
      {/* Resume Notification Component - Displayed randomly */}
      {resumeNotification && (
        <ResumeNotification 
